...
export default function Edit({ match }) {
  ...
  const hanldeSubmit = async e => {
   ...
   try {
    ...
    const res = await axios.put(`/newsEditors/${match.params.id}`, formData);
  ...
  const handleKhongPheDuyet = async e => {
   e.preventDefault();
   try {
      const formData = new FormData();
   ...
   const res = await axios.put(`/newsEditors/unpublished/${match.params.id}`, formData);
const { code, message } = res.data;
    ...
    <div className="row">
     <div className="col-xl-12 grid-margin">
        <form onSubmit={hanldeSubmit} className="w-100">
         ...
         <button type="button" className="btn btn-danger bety-btn" onClick={handleKhongPheDuyet}>
            KHÔNG PHÊ DUYỆT
            </button>
        </form>
     </div>
