...
function setAndSendResponse(res, responseCode) {
    return res.status(responseCode.statusCode).send(responseCode.body);
}

function callRes(res, responseCodeName, data = null) {
  if (responseCodeName != responseCode.OK){
    let x = {
      code: responseCodeName.body.code,
      message: responseCodeName.body.message,
      details: null
    }
    if (data) x.details = data.toString();
    return res.status(responseCodeName.statusCode).send(convertString(x));
  }
  else {
    let x = {
      code: responseCodeName.body.code,
...
module.exports = {responseCode, setAndSendResponse, callRes};